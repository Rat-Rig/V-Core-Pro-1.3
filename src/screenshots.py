...
try:
    while True:
        ...
        for file_path in paths[-repeats:]:
            ...
            subprocess.call(
                shlex.split(
                    f"convert '{file_path}' -resize 1200x800 -background white -alpha remove -quality 90 '{new_path}'"
                )
            )
            os.remove(file_path)

        shutil.rmtree(
            os.path.join(BASE_PATH, "gifs", assembly_name),
            ignore_errors=True,
        )
        shutil.move(assembly_dir, os.path.join(BASE_PATH, "gifs"))
